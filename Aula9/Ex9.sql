...
BEGIN
    -- SELECT ACTUAL HD AND RAM FROM DEVICE
    ...
    -- SELECT THE TOTAL HD AND RAM WITH ALL SOFTWARES IN THE DEVICE
    SELECT COALESCE(SUM(HARDDISK), 0), 
    COALESCE(SUM(MEMORIA_RAM), 0)
    INTO SUM_HD, SUM_RAM
    FROM SOFTWARE
    WHERE FK_MAQUINA = ID_MAQ;

    -- CONDITIONALS TO VERIFY IF THE DEVICE NEEDS AN UPGRADE
    IF SUM_HD <= HD AND SUM_RAM <= RAM THEN
        RAISE NOTICE 'A MÁQUINA % ESTÁ COM RECURSOS SUFICIENTES', ID_MAQ;
    ELSE
        IF SUM_HD > HD THEN
            RAISE NOTICE 'A MÁQUINA % PRECISA DE UPGRADE DE HARDDISK DE %GB', ID_MAQ, SUM_HD - HD;
        END IF;
        IF SUM_RAM > RAM THEN
            RAISE NOTICE 'A MÁQUINA % PRECISA DE UPGRADE DE MEMÓRIA RAM DE %GB', ID_MAQ, SUM_RAM - RAM;
        END IF;
    END IF;
END;
$$ LANGUAGE plpgsql;
